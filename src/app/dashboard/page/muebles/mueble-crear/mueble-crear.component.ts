...
export class MuebleCrearComponent implements OnInit {
  ...
  getMueblePorId(id: number) {
    ...
    this._serviceMueble.getMuebleporId(id).subscribe(({ data }: IResponse) => {
      ...
      this.fechaEstimadaEntrega.setValue(this.datePipe.transform(data.fechaEstimadaEntrega, "yyyy-MM-dd"));
  ...
  getUsuariosAsignadosMueble(id: number) {
    this._serviceMueble.getUsuariosAsignadosMueble(id).subscribe(({ data }: IResponse) => {
      this.usuariosSeleccionados = data;
      for (let i = 0; i < this.usuariosSeleccionados.length; i++) {
        this.usuarios.find(x => x.idUsuario == this.usuariosSeleccionados[i].idUsuario).seleccionar = true;
      }
    });
  }

  getMateriaPrimaAsignadosMueble(id: number) {
    this._serviceMueble.getMateriaPrimaAsignadosMueble(id).subscribe(({ data }: IResponse) => {
      this.materiasPrimasSeleccionados = data;
      for (let i = 0; i < this.materiasPrimasSeleccionados.length; i++) {
        this.materiasPrimas.find(x => x.idMateriaPrima == this.materiasPrimasSeleccionados[i].idMateriaPrima).seleccionar = true;
      }
    });
  }

  getUsuariosParaMuebles() {
    this._serviceUsuario.getUsuariosParaMueble().subscribe((response: IResponse) => {
  ...
  getMateriaPrima() {
    this._serviceMateriaPrima.getMateriaPrima().subscribe((reponse: IResponse) => {
  ...
  guardar() {
    ...
    this._serviceMueble.postInsertarMueble(mueble).subscribe(({ data }: IResponse) => {
      let listadoMueble = {
        usuarios: [...this.usuariosSeleccionados],
        materiaPrima: [...this.materiasPrimasSeleccionados],
        idMueble: data.idMueble
      }
      this._serviceMueble.postInsertarMuebleListados(listadoMueble).subscribe(data => {
        this._serviceMensajes.successMessage('Ok', 'Mueble creado con exito');
        this.spinner.hide();
  ...
  editar() {
    ...
    this._serviceMueble.putActualizarMueble(mueble).subscribe(data => {
      ...
      this._serviceMueble.postInsertarMuebleListados(listadoMueble).subscribe(data => {
        this._serviceMensajes.successMessage('Ok', 'Mueble actualizado  con exito');
        this.spinner.hide();
        this.router.navigate(['dashboard/mueble/listar']);
      });
  ...
  seleccionarUsuarios() {
    ...
    if (usuariosS.length > 0) {
      ...
      this._serviceMensajes.errorMessage('', 'Seleccione por lo menos un usuario', '');
    }
  }

  seleccionarMateriaPrima() {

    let materiasPrimasS = this.materiasPrimas.filter(x => x.seleccionar == true);
    if (materiasPrimasS.length > 0) {
      this.materiasPrimasSeleccionados = materiasPrimasS;
      this.materiasPrimasSeleccionados = this.materiasPrimasSeleccionados.map(data => {
        return { ...data, unidadesDisponibles: 0 }
      })
    } else {
      this._serviceMensajes.errorMessage('', 'Seleccione por lo menos una materia prima', '');
    }
  }
  ...
  obtenerCostoMueble() {
    ...
    let sumaMateria = 0;
    for (let i = 0; i < this.materiasPrimasSeleccionados.length; i++) {
      sumaMateria = sumaMateria + (this.materiasPrimasSeleccionados[i].valor * this.materiasPrimasSeleccionados[i].unidadesDisponibles);
    }
  ...
  get nombre(): AbstractControl { return this.formMueble.get('nombre'); }
  get descripcion(): AbstractControl { return this.formMueble.get('descripcion'); }
  get nombreCliente(): AbstractControl { return this.formMueble.get('nombreCliente'); }
  get costoMateriaPrima(): AbstractControl { return this.formMueble.get('costoMateriaPrima'); }
  get fechaEstimadaEntrega(): AbstractControl { return this.formMueble.get('fechaEstimadaEntrega'); }
  get tipo(): AbstractControl { return this.formMueble.get('tipo'); }
  get fechaFabricacion(): AbstractControl { return this.formMueble.get('tipfechaFabricaciono'); }
}
