...
export class LoginComponent implements OnInit {
  ...
  login() {
    ...
    this._serviceAutenticacion.postAutenticacion(this.formAutenticacion.value).subscribe((data: IResponse) => {
      if (data.error == 200) {
        sessionStorage.setItem('user', JSON.stringify(data));
        this._serviceMensajes.successMessage('Login', data.mensaje);
